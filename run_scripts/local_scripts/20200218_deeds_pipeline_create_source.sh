...
preprocessed_image_folder=${DATA_ROOT}/source_image/preprocess
...
#  label=${atlas_label}
  ...
  set -o xtrace
  ${PYTHON_ENV} ${SRC_ROOT}/tools/reg_thorax_non_rigid.py --fixed ${fixed_img} --moving ${moving_img} --out ${out_img} --omat ${omat_txt} --reg_tool_root ${reg_tool_root} --reg_method ${reg_method} --reg_args ${reg_args}
  set +o xtrace
