...
run_process_one_scan () {
  ...
  local MASK_IMG=${REFERENCE_FOLDER}/valid_region_mask.nii.gz
  ...
  run_registration_pipeline () {
    # Run with high resolution, with more control on stability.
    run_high_res () {
      set -o xtrace
      ln -sf ${MASK_IMG} ${OUTPUT_HIGH_RES_FOLDER}/mask.nii.gz
      ln -sf ${MOVING_IMG} ${OUTPUT_HIGH_RES_FOLDER}/moving.nii.gz
      set +o xtrace
      run_registration_pipeline_res ${OUTPUT_HIGH_RES_FOLDER} "${corrField_config_step1}" "${effective_etch_step1}"
    }

    run_combined_jacobian_analysis () {
      set -o xtrace
      cp ${OUTPUT_HIGH_RES_FOLDER}/output/trans.nii.gz \
        ${OUTPUT_JAC_FOLDER}/trans_combine.nii.gz

      ${NIFYREG_ROOT}/reg_jacobian \
        -trans ${OUTPUT_JAC_FOLDER}/trans_combine.nii.gz \
        -ref ${MASK_IMG} \
        -jac ${OUTPUT_JAC_FOLDER}/jac_full_res.nii.gz
    ...
    set -o xtrace
    cp ${OUTPUT_HIGH_RES_FOLDER}/output/warp.nii.gz \
      ${OUTPUT_INTERP_FOLDER}/ori/${scan_name}
    set +o xtrace
  }
