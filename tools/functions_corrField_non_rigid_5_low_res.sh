run_process_one_scan () {
  ...
  local OUTPUT_LOW_RES_4_FOLDER=${scan_root_folder}/output_low_res4
  local OUTPUT_LOW_RES_5_FOLDER=${scan_root_folder}/output_low_res5
  ...
  mkdir -p ${OUTPUT_LOW_RES_4_FOLDER}
  mkdir -p ${OUTPUT_LOW_RES_5_FOLDER}
  ...
  run_registration_pipeline () {
    ...
    # 3. Run with updated mask still with low resolution.
    run_low_res_3 () {
      set -o xtrace
    ...
    # 4. Run with updated mask still with low resolution.
    run_low_res_4 () {
      set -o xtrace
      ln -sf ${OUTPUT_LOW_RES_3_FOLDER}/mask.nii.gz ${OUTPUT_LOW_RES_4_FOLDER}/mask.nii.gz
      ln -sf ${OUTPUT_LOW_RES_3_FOLDER}/output/warp.nii.gz ${OUTPUT_LOW_RES_4_FOLDER}/moving.nii.gz
      set +o xtrace
      run_registration_pipeline_res ${OUTPUT_LOW_RES_4_FOLDER} "${corrField_config_step4}" "${effective_etch_step4}"
    }

    # 5. One additional low res step to make the search radius increase gradually.
    run_low_res_5 () {
      set -o xtrace
      ln -sf ${OUTPUT_LOW_RES_4_FOLDER}/mask.nii.gz ${OUTPUT_LOW_RES_5_FOLDER}/mask.nii.gz
      ln -sf ${OUTPUT_LOW_RES_4_FOLDER}/output/warp.nii.gz ${OUTPUT_LOW_RES_5_FOLDER}/moving.nii.gz
      set +o xtrace
      run_registration_pipeline_res ${OUTPUT_LOW_RES_5_FOLDER} "${corrField_config_step5}" "${effective_etch_step5}"
    }

    # 6. Run with high resolution, with more control on stability.
    run_high_res () {
      set -o xtrace
      ...
      ${NIFYREG_ROOT}/reg_transform \
        ...
        ${OUTPUT_HIGH_RES_FOLDER}/trans_combine_1_2.nii.gz
      ${NIFYREG_ROOT}/reg_transform \
        -comp \
        ${OUTPUT_HIGH_RES_FOLDER}/trans_combine_1_2.nii.gz \
        ${OUTPUT_LOW_RES_3_FOLDER}/output/trans.nii.gz \
        ${OUTPUT_HIGH_RES_FOLDER}/trans_combine_1_2_3.nii.gz
      ${NIFYREG_ROOT}/reg_transform \
        -comp \
        ${OUTPUT_HIGH_RES_FOLDER}/trans_combine_1_2_3.nii.gz \
        ${OUTPUT_LOW_RES_4_FOLDER}/output/trans.nii.gz \
        ${OUTPUT_HIGH_RES_FOLDER}/trans_combine_1_2_3_4.nii.gz
      ${NIFYREG_ROOT}/reg_transform \
        -comp \
        ${OUTPUT_HIGH_RES_FOLDER}/trans_combine_1_2_3_4.nii.gz \
        ${OUTPUT_LOW_RES_5_FOLDER}/output/trans.nii.gz \
      ...
      set +o xtrace
      run_registration_pipeline_res ${OUTPUT_HIGH_RES_FOLDER} "${corrField_config_step6}" "${effective_etch_step6}"
    }
    ...
    set -o xtrace
    cp ${OUTPUT_HIGH_RES_FOLDER}/output/warp.nii.gz ${OUTPUT_INTERP_FOLDER}/ori/${scan_name}
    set +o xtrace
  }
