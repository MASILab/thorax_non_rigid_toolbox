run_process_one_scan () {
  ...
  local FINAL_FOLDER=${OUTPUT_LOW_RES_3_FOLDER}
...
run_interp_scan () {
  ...
  local low_res_3_trans=${OUTPUT_LOW_RES_3_FOLDER}/output/trans.nii.gz
  ...
  run_interp_scan_flag () {
    ...
    ${NIFYREG_ROOT}/reg_resample \
      ...
      -res ${out_mask}_1.nii.gz

    ${NIFYREG_ROOT}/reg_resample \
      -inter 0 -pad 0 \
      -ref ${REF_MASK_IMG} \
      -flo ${out_mask}_1.nii.gz \
      -trans ${low_res_2_trans} \
      -res ${out_mask}_2.nii.gz

    ${NIFYREG_ROOT}/reg_resample \
      -inter 0 -pad 0 \
      -ref ${REF_MASK_IMG} \
      -flo ${out_mask}_2.nii.gz\
      -trans ${low_res_3_trans} \
      -res ${out_mask}

    rm ${out_mask}_1.nii.gz
    rm ${out_mask}_2.nii.gz
    set +o xtrace
  }

  get_effective_region () {
    local out_mask_folder=${OUTPUT_INTERP_FOLDER}/effective_region
    local final_effective_region_mask=${OUTPUT_LOW_RES_3_FOLDER}/output/effective_mask.nii.gz
