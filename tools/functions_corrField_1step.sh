...
#      set +o xtrace
#      run_registration_pipeline_res ${OUTPUT_HIGH_RES_FOLDER} ""
#    }

    run_combined_jacobian_analysis () {
      set -o xtrace
      cp ${OUTPUT_LOW_RES_FOLDER}/output/trans.nii.gz ${OUTPUT_JAC_FOLDER}/trans_combine.nii.gz
...
#        ${OUTPUT_LOW_RES_FOLDER}/output/effective_mask.nii.gz \
#        -overlap 1
    ...
    ${NIFYREG_ROOT}/reg_resample -inter 3 -pad NaN \
      ...
      -flo ${MOVING_IMG} \
      -trans ${OUTPUT_JAC_FOLDER}/trans_combine.nii.gz \
      -res ${OUTPUT_INTERP_FOLDER}/ori/${scan_name}
    set +o xtrace
  }
...
run_interp_scan () {
  ...
  local scan_root_folder=${OUTPUT_ROOT_FOLDER}/${scan_name_no_ext}
  local OUTPUT_JAC_FOLDER=${scan_root_folder}/jac
  local scan_trans=${OUTPUT_JAC_FOLDER}/trans_combine.nii.gz
  ...
  get_effective_region () {
    ...
    local final_effective_region_mask=${scan_root_folder}/output_low_res/output/effective_mask.nii.gz
    ...
    ${NIFYREG_ROOT}/reg_resample \
      -inter 0 -pad 0 \
      -ref ${MASK_IMG} \
      -flo ${final_effective_region_mask} \
      -res ${out_mask_folder}/${scan_name}
    set +o xtrace
  }
