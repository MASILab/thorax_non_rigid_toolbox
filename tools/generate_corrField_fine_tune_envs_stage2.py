...
corrField_config_step2="-L {search_radius_opt_str} -a {reg_opt_str} -N {kp_disp_opt_str} -R {sim_size_opt_str}"
...
def generate_test_env(fine_tune_case_root):
    ...
    range_kp_disp = np.arange(4, 17, 3, dtype=int)
            ...
            for idx_reg in test_idx_range_reg:
                for idx_sim_size in test_idx_range_sim_size:
                    case_folder = os.path.join(fine_tune_case_root,
                                               f'step2_{idx_search_radius}_{idx_kp_disp}_{idx_reg}_{idx_sim_size}')
                    mkdir_p(case_folder)
